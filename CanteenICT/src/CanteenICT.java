// File: CanteenICT.java
// Description: This is a canteen that use to collect optional code in pre and post process
// Project: 1
//
// ID: 6588034
...
public class CanteenICT {
	...
	{	
		...
		for(FoodStall a: this.foodStalls) {
			for(FoodStall.Menu d: a.getMenu()) {
				for(int j=0;j<FoodStall.Menu.values().length;j++)
					if(FoodStall.Menu.values()[j].equals(d)) {
						sum += FoodStall.INSTALLATION_COST[j];
					}
			}
		}
		return sum;
	...
	{
		if(getInstallCost() >= MAX_BUDGET) {
				return false;
		}
		if(tables.size() == 0) {
			return false;
		}
		for(FoodStall i: this.foodStalls) {
			if(i.getMenu().size()==5) {
				return true;
			}else {return false;}
		}
	...
	private void postprocess()
	{
		...
		for(int i=0;i<foodStalls.size();i++) {
			for(int j=0;j<foodStalls.get(i).getCustomerQueue().size();j++) {
				if(waitToSeatQueue.contains(foodStalls.get(i).getCustomerQueue().get(j))) {
					foodStalls.get(i).getCustomerQueue().remove(j);
				}
			}
		}
		if(cloneQ.size() !=0) {
			waitToEnterQueue.removeAll(cloneQ);
		}
		for(int i=0;i<tables.size();i++) {
			if(tables.get(i).getSeatedCustomers().size() != 0) {
				for(int j=0;j<doneQueue.size();j++) {
					if(tables.get(i).getSeatedCustomers().get(0).equals(doneQueue.get(j))) {
						tables.get(i).getSeatedCustomers().remove(doneQueue.get(j));
					}	
				}
			}
		}
		
		cloneQ.removeAll(allCustomers);
		//****************************************************************************************************//
	...
	public List<FoodStall> getFS()  {
		return foodStalls;
	}
	public List<Table> getTB(){
		return tables;
	}
	...
	private List<Customer> clone =  new ArrayList<Customer>();
	
	public List<Customer> getClone(){
		return clone;
	}
	private List<Customer> cloneQ =  new ArrayList<Customer>();
	public List<Customer> getCloneQ(){
		return cloneQ;
	}
	//******************************************** DO NOT MODIFY ****************************************//
